...
- **数据和模型：** 由于涉及到的模型及数据的大小较小（几十M），因此已被打包到镜像（容器）目录 /TS-Knight/Quantize/Onnx/example下；
...
- __后处理__ 
  ...
  - 也可3个网络分开执行后处理脚本：
    - bash scripts/postprocess_det.sh 
    - bash scripts/postprocess_cls.sh 
    - bash scripts/postprocess_rec.sh
