...
### 2. 编译
    ...
    Knight --chip TX5368AV200 rne-compile --onnx alexnet_quantize.onnx --outpath alexnet_example/compile_result


### 3. 仿真

    #准备bin数据
    python3 make_alexnet_input_bin.py.py  
    #仿真
    Knight --chip TX5368A rne-sim --input input.bin --weight alexnet_quantize_r.weight --config  alexnet_quantize_r.cfg --outpath alexnet_example

### 4. 性能分析

```
Knight --chip TX5368A rne-profiling --weight  alexnet_r.weight --config  alexnet_r.cfg --outpath  alexnet_example/
```
