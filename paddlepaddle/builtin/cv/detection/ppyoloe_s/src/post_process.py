...
def detect(save_img=True):
    ...
    for step_id, data in enumerate(tqdm(loader)):
        # forward
        outs = {}
        cls_score_list = np.load(numpys[0])*scales[0]
        reg_lrtb_list = np.load(numpys[1])*scales[1]
        cls_score_list = torch.from_numpy(cls_score_list).squeeze(-1)
        reg_lrtb_list = torch.from_numpy(reg_lrtb_list).squeeze(-1)
        print(reg_lrtb_list.shape)
