...
def infer_ocr_det_model(executor):
    ...
    det_test_data_100 = paddle.load(executor.dataset)
    paddle.save(det_test_data_100[:10], 'det_b1_10.pd')
    #print('****',len(det_test_data_100))
...
def infer_ocr_cls_model(executor):
    ...
    data format:
        ...
        label: B
    """
    cls_test_30_b1 = paddle.load(executor.dataset)
...
def infer_ocr_rec_model(executor):
    ...
    data format:
        ...
        ignoreT: 128
    """
    uds_path = os.path.dirname(executor.user_defined_script)
    rec_test_data = paddle.load(executor.dataset)
    paddle.save(rec_test_data[:10], 'rec_b1_10.pd')
    post_process_class = CTCLabelDecode(
        character_dict_path=os.path.join(uds_path,"paddle_ocr/infer/en_dict.txt"),
        use_space_char=True
    )
