...
@onnx_infer_func.register("infer_yoloworld")
def infer_yoloworld(executor):
    ...
    parser.add_argument(
        '--outpath',
        type=str,
        default='/TS-KnightDemo/output/yolov5s/rne',
        required=True,
        help='the out image.')
    ...
    outpath = executor.save_dir
    project_name = "yoloWord"
    model = "yoloworld"
    ...
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    input_data = runModel.load_image(image_path,  outpath)

    outputs = executor.forward(*(input_data, text_feats))
