...
def detect(onnxpath, dataset, save_dir='./output'):
    ...
    for step_id, data in enumerate(tqdm(loader)):
        ...
        #input_data = input_data.astype(np.uint8)
        #cls_score_list, reg_lrtb_list = executor.forward(input_data)
        cls_score_list, reg_lrtb_list = session.run(output_names=[output_name_0, output_name_1], input_feed={input_name: input_data})
...
if __name__ == '__main__':
    ...
    onnxpath = '../weight/yolonas_s.onnx'
    dataset = 'coco128.yaml'
    detect(onnxpath, dataset, save_dir='./output')