...
def yolov6s(weight_path=None):
    ...
    ckpt = torch.load(weight_path, map_location='cpu')  # load
...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task, executor, iteration=10000):
        '''Model prediction
...
def yolov6s(executor):
    ...
    #_ = val.init_model(None, weights, task)
    dataloader = val.init_data(None, task)

    model = None
    pred_result, vis_outputs, vis_paths = val.predict_model(model, dataloader, task, executor, iteration)
