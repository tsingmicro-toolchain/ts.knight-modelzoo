...
def detect(save_img=True):
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        cls0 = np.load(numpys[0])*scales[0]
        cls1 = np.load(numpys[1])*scales[1]
        cls2 = np.load(numpys[2])*scales[2]

        count += 1
        # raise
        cls0 = torch.from_numpy(cls0).float()
        cls1 = torch.from_numpy(cls1).float()
        cls2 = torch.from_numpy(cls2).float()
